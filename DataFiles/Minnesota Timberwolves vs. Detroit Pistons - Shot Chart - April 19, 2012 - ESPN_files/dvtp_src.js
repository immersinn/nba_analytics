...
Array.prototype.dvContains = function (obj) {
    var i = this.length;
    while (i--) {
        if (this[i] === obj) {
            return true;
...
try {
    ...
    for (dv_i in dv_scripts) {
        ...
        if (dv_src && dv_src.match(/dvtp_src?\.js/) && !_dvScripts.dvContains(dv_scripts[dv_i])) {
            ...
            if (false == (/MSIE (\d+\.\d+);/.test(navigator.userAgent) && (new Number(RegExp.$1) <= 8) && (dv_additionalUrlParam.length + dv_src.length > 2000))) {
            ...
            var dv_isChrome = navigator.userAgent.toLowerCase().indexOf('chrome') > -1;
            if (dv_isChrome) {
                dv_src += '&referrer=' + encodeURIComponent(dv_iframeDocument.referrer);
            }
